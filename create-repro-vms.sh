...
EOF
echo "rm -r ./reprologs" >> $ahareproscript
echo "mkdir -p ./reprologs" >> $ahareproscript
for region in ${regions[@]}; do
    ...
    for subscription in ${subscriptions[@]}; do
        ...
        az network public-ip list --query "[?resourceGroup=='tlsrepro$region'].ipAddress" -o tsv | xargs -L 1 -I {} echo "bash -c \"ssh -o 'StrictHostKeyChecking no' azureuser@{} 'chmod +x ~/repro.sh && ~/repro.sh' > './reprologs/{}.log'\" &" >> $ahareproscript
    done
done
